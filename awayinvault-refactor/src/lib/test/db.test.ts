...
describe("Supabase RLS Policies Integration Test", () => {
	...
	beforeAll(async () => {
		...
		// Initialize two clients representing different users
	...
	it("should allow User A to create, read, update, and delete their own vault items", async () => {
		...
		const { error: deleteError } = await clientA.from("vault_items").delete().eq("id", itemId);

		expect(deleteError).toBeNull();

		// Verify item is deleted
		const { data: verifyData } = await clientA.from("vault_items").select("*").eq("id", itemId);

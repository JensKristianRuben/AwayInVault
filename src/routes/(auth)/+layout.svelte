	...
	<div class="relative w-full max-w-4xl h-[650px] md:h-[600px] overflow-hidden">
		...
		<div
			class="absolute left-0 top-0 w-full md:w-1/2 h-full flex flex-col justify-center p-8 transition-all duration-700 ease-in-out z-10
             {isRegister
				? 'opacity-0 pointer-events-none -translate-x-full md:translate-x-12'
		...
		<div
			class="absolute left-0 top-0 w-full md:w-1/2 h-full flex flex-col justify-center p-8 transition-all duration-700 ease-in-out z-10
             {isRegister
				? 'opacity-100 translate-x-0 md:translate-x-full'
				: 'opacity-0 pointer-events-none translate-x-full md:translate-x-[-12]'}"
		>
			<div class="max-w-md w-full mx-auto">
				<!-- Header -->
				...
				<!-- Form REGISTER -->
				<form class="space-y-4" method="POST" action="/register?/register">
					<div class="space-y-2">
						<label
							for="reg-email"
							class="text-xs font-semibold uppercase tracking-wider text-text-muted ml-1"
		...
		<div
			class="hidden md:flex absolute top-0 left-0 w-1/2 h-full bg-bg-sidebar flex-col items-center justify-center p-12 text-center transition-all duration-700 ease-in-out z-20
             {isRegister
				? 'translate-x-0 border-r border-border-subtle'
				: 'translate-x-full border-l border-border-subtle'}"
		>
			{#if isRegister}
				<div class="space-y-6">
